...
target_temp = 300.0         # K
temp_tol = 2.0               # K
pressure_tol = 200.0         # bar, NPT pressure is noisy
...
for key, filename in files.items():
    ...
    plt.xlabel('Time (ps)')
    plt.ylabel(key)
    plt.title(f'{key} vs Time (NPT)')
    plt.grid(True)
    plt.tight_layout()
    png_name = f"npt_{key.lower()}.png"  # <-- Save images with npt_ prefix
