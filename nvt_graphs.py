...
# Configuration: NVT files
files = {
    "Temperature": "nvt_temperature.xvg",
    "Pressure": "nvt_pressure.xvg",
    "Potential": "nvt_potential.xvg"
}

# Recommended thresholds
target_temp = 300.0         # K
temp_tol = 2.0               # K
pressure_tol = 200.0         # bar, NVT pressure may fluctuate
...
for key, filename in files.items():
    ...
    plt.xlabel('Time (ps)')
    plt.ylabel(key)
    plt.title(f'{key} vs Time (NVT)')
    plt.grid(True)
    plt.tight_layout()
    png_name = f"nvt_{key.lower()}.png"
...
if all_ok:
    final_comment = "NVT equilibration looks stable. Safe to proceed to NPT or production run."
else:
    final_comment = ("NVT equilibration shows instabilities. "
                     "Check comments above and consider longer equilibration, "
                     "adjusting thermostat or timestep.")
